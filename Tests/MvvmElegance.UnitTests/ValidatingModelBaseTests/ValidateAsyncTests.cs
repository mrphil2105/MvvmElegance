...
public class ValidateAsyncTests
{
    ...
    public async Task ValidateAsync_AddsErrors_WhenValidatorReturnsErrors(
        Dictionary<string, IEnumerable<string>> errors, [Frozen] Mock<IModelValidator> validatorMock,
        ValidatingModel model)
    {
        ...
        model.HasErrors.Should()
            .BeTrue();
        errors.Keys.ToDictionary(pn => pn, model.GetErrors)
            .Should()
    ...
    public async Task ValidateAsync_AddsNonErrors_WhenValidatorReturnsNonErrors(
        Dictionary<string, IEnumerable<string>>? nonErrors, [Frozen] Mock<IModelValidator> validatorMock,
        ValidatingModel model)
    {
        validatorMock.Setup(v => v.ValidateAsync(default))
        ...
        model.HasErrors.Should()
            .BeFalse();
        nonErrors?.Keys.ToDictionary(pn => pn, model.GetErrors)
            .Should()
    ...
    {
        validatorMock.Setup(v => v.ValidateAsync(default))
            .ReturnsAsync(new Dictionary<string, IEnumerable<string>>());
        await model.ValidateAsync();
        validatorMock.Setup(v => v.ValidateAsync(default))
            .ReturnsAsync(errors);
